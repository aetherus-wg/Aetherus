...
pub fn scatter<R: Rng>(rng: &mut R, phot: &mut Photon, env: &Local) -> EventId {
    ...
    EventId { event_type: EventType::MCRT(mcrt_event!(Material, Elastic, HenyeyGreenstein, Unknown)), src_id: env.mat_id() }
}
...
pub fn shift_scatter<R: Rng>(rng: &mut R, phot: &mut Photon, env: &Local) -> EventId {
    ...
    if r <= env.shift_prob() {
        ...
        *phot.weight_mut() = 0.0;
        return EventId { event_type: EventType::MCRT(mcrt_event!(Material, Inelastic, Raman, Unknown)), src_id: env.mat_id() }
    }
