use crate::{
    ...
    clone,
    core::Real,
    math::stat::CumulativeDistributionFunction, ord::Spherical,
};
...
impl SphericalCdfPlane {
    ...
    pub fn azimuthal_angle_in_plane(&self, azimuthal_angle: Real) -> bool {
        ...
        if azimuthal_angle + offset_angle >= self.azimuth_angle + offset_angle - half_dazimuth && azimuthal_angle + offset_angle < self.azimuth_angle + offset_angle + half_dazimuth  {
            true 
        } else {
            false
        }
...
impl SphericalCdf {
    ...
    pub fn sample<R: Rng>(&self, rng: &mut R) -> (Real, Real) {
        ...
        let iplane = self.planes.iter().position(|pl| pl.azimuthal_angle_in_plane(azim_draw)).unwrap();
        println!("{}", iplane);
...
impl From<PhotometricWeb> for SphericalCdf {
    ...
    fn from(photweb: PhotometricWeb) -> SphericalCdf {
        ...
        let cdf_planes = photweb.planes().iter().enumerate()
        .map(|(iplane, plane)| {
            ...
            let probs = plane.intensities().iter().map(|intens| intens / plane_intensity).collect();
            ...
            *curr_cdf_plane.cdf_mut() = CumulativeDistributionFunction::from_pdf(probs, plane.angles().clone().to_vec());
        ...
        let azim_angles = photweb.planes().iter().map(|plane| plane.angle()).collect::<Vec<Real>>();
        let azim_probs = photweb.planes().iter()
            .map(|plane| {
                plane.integrate_intensity() / total_intens
            })
            .collect::<Vec<Real>>();
