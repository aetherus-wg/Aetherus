...
{
    let s = read_to_string(path).context(format!("Failed reading from {}", path.display()))?;
...
mod test {
    ...
    const JSON_STR: &str =
        "{ \"num_prop\": 42, \"string_prop\": \"Lorem Ipsum\", \"boolean_prop\": true }";
    ...
    fn test_string_deserialisation() {
        ...
        assert_eq!(
            ...
            TestStruct {
                num_prop: 42,
                string_prop: "Lorem Ipsum".to_string(),
                boolean_prop: true
            }
        );
    }
